...
class CreateMinMaxResultsTbl:
    ...
    def run(self):
        ...
        with self.__engine.begin() as conn:
            self.__log_info(f'Retrieving columns from {global_vars.V_PATIENT_DATA_RESULT_NUM}...')
            cols = get_columns_from_table(conn, global_vars.V_PATIENT_DATA_RESULT_NUM)
    ...
    def __create_table_select_query(self, cols: list):
        # Comments regarding the queries have an arrow (->) indicating the part of the query that will be added in the next
        ...
        query = query + ', '.join(dia_cols_query) + f' from {global_vars.V_PATIENT_DATA_RESULT_NUM}' \
        ...
        query = query + ', '.join(manha_cols_query) + f' from {global_vars.V_PATIENT_DATA_RESULT_NUM} where DIA_UCI is not null' \
    ...
    def __create_view_query(self, patient_data_cols: list, added_cols: list) -> str:
        ...
        query = f'create view {global_vars.V_PATIENT_DATA_MIN_MAX_RESULT} as select {global_vars.TBL_PATIENT_MIN_MAX_RESULT}.ID_PACIENTE, DT_COLHEITA, {", ".join(patient_data_cols)}, {", ".join(added_cols)} ' \
                f'from {global_vars.TBL_PATIENT_MIN_MAX_RESULT} join {global_vars.V_PATIENT_DATA_RESULT_NUM} ' \
                f'on {global_vars.TBL_PATIENT_MIN_MAX_RESULT}.ID_PACIENTE = {global_vars.V_PATIENT_DATA_RESULT_NUM}.ID_PACIENTE ' \
